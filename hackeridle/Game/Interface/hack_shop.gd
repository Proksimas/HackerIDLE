...
func set_shop():
	...
	for item_name in HackingItemsDb.hacking_items_db:
		...
		if item_present.has(item_name) and Player.has_hacking_item(item_name):
			...
			new_hack_item.s_wait_too_long.connect(\
				get_tree().get_root().get_node("Main/Interface")._on_s_wait_too_long.bind(true))
...
func player_bought_hacking_item(item_name,  quantity):
	...
	if not Player.has_hacking_item(item_name):
		...
		cost = StatsManager.calcul_hack_stat(StatsManager.Stats.COST,Calculs.total_hacking_prices(item_cara, 1))
		...
		cost =  StatsManager.calcul_hack_stat(StatsManager.Stats.COST,Calculs.total_hacking_prices(Player.hacking_item_bought[item_name], quantity))
